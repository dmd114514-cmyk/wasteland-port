...
### 第十二轮：边缘衔接 + 密度提升 + 重叠修复（2026-08-28）

新档实测三问题：城市平原与周边地形是垂直断面（无衔接）、建筑太少、
个别建筑贴边/重叠（Ruin001 右偏 1 格）。

1. **边缘渐变衔接**（`flattenCity`）：整平范围外扩 **8 格过渡带**，带内
   高度从平原基准（base）线性渐变回自然地形（`target = base +
   (gy−base)*dist/blend`），城市边缘是缓坡而不是悬崖；带外地形不动。
   冒烟输出 `City flatten: base=64 blend=8`。
2. **密度提升**（`RuinedCity`）：
   - 子块建筑目标 **4/4 全放**（原中心 3-4、外围 2-4）——密度直接上限；
   - 子块内加 **±1 随机交错偏移**——相邻子块错位，占用冲突减少、密度再升；
   - 子块放置概率 80/70 → **90/85**；大池街区概率 15% → **25%**。
3. **重叠修复**：`Ruin001` 实际摆幅比锚点右偏 1 格（实测 anchor x+1 起摆、
   最大 x+12）→ 放置时 `x0--` 左移 1，占用登记与真实摆幅对齐；其余 7 类
   组件尺寸与占用一致，交错偏移控制在子块内（不超边距 9）→ 不重叠。
4. 冒烟（NTM-CE 服）：`blocks=36 biomeFail=0 groundFail=0 createFail=0
   buildings=91 hbm=8 hbmLoaded=true`（无异常、无 skip、地堡 Y:62）——
   建筑数 59→65→**91**（密度提升生效，零失败计数说明占用校验全程拦截）。
- 交付：`build/libs/wasteland-1.0.0.jar`（241,708 B，备份 `.bak-20260828-14`）。