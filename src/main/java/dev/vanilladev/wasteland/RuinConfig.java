...
public class RuinConfig
{
	...
	public static void load(Configuration config)
	{
		...
		rareRuinLootChance = config.getInt("Uncommon ruin chest loot chance", "Loot Rarity", 8, 1, 10000, "If not picked, defaults to common ruin chest loot");
	...
	public static CustomItemStack[] getLoot(String[] rawStringArray)
	{
		...
		for (int i = 0; i < rawStringArray.length; i++)
		{
			if (rawStringArray[i].length() > 0)
			{
				...
				Item itemRef = Item.getByNameOrId(mod + ":" + item);
				items[i] = new CustomItemStack(itemRef == null ? ItemStack.EMPTY : new ItemStack(itemRef, 1), max, min);
			}
		}
