package dev.vanilladev.wasteland.utils;
...
public class CustomItemStack 
{
	...
	public static ItemStack[] getLootItems(Random rand, CustomItemStack[] items, int minItems, int maxItems, boolean repeat)
	{
		...
		{
			temp_items[i] = new CustomItemStack(items[i].items.copy(), items[i].maxItems, items[i].minItems);
		}
	...
	public static void placeLoot(Random rand, TileEntityChest chest, ItemStack[] loot)
	{
		for (int i = 0; i < loot.length; i++)
		{
			...
			int checked = 0;
			while (!chest.getStackInSlot(slot).isEmpty() && checked < chest.getSizeInventory())
			{
				slot = slot + 1;
			...
			if (checked >= chest.getSizeInventory())
			{
				break;
			}
			if (loot[i] != null && !loot[i].isEmpty())
			{
